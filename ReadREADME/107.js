var map=[['-','-','-','427','668','495','377','678','-','177','-','-','870','-','869','624','300','609','131','-','251','-','-','-','856','221','514','-','591','762','182','56','-','884','412','273','636','-','-','774'],
...
function getST(a){
  var pArr=[0],edges=[],i,j,ti,tj,mi,mj;
  while(pArr.length<a.length){
    mi=mj=-1;
    for(i=0;i<a.length;i++) for(j=0;j<i;j++){
      if(a[i][j]=='-') continue;
      ti=i;tj=j;
      if(pArr.indexOf(i)==-1){ti=j;tj=i;}
      if(pArr.indexOf(ti)>=0&&pArr.indexOf(tj)==-1){
        if(mi==-1||~~a[mi][mj]>~~a[ti][tj]){mi=ti;mj=tj;}
      }
    }
    pArr.push(mj);
    edges.push([mi,mj]);
  }
  for(i=j=0;i<edges.length;i++) j+=~~a[edges[i][0]][edges[i][1]];
  return j;
}
for(var x=getST(map),y=0,i=0,j;i<map.length;i++) for(j=0;j<i;j++)
